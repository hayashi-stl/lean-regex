...
theorem matchPartial_or_terminates {q r : Regex α} {s : Pos w} {cap : Captures w}
...
theorem matchPartial_filterEmpty_terminates {emp : Bool} {r : Regex α}
    {s : Pos w} {cap : Captures w}
...
theorem matchPartial_capture_terminates {n : ℕ} {r : Regex α}
    {s : Pos w} {cap : Captures w}
...
theorem matchPartial_concat (q r : Regex α) (s : Pos w) (cap : Captures w)
    ...
    : matchPartial [/⟨q⟩ ⟨r⟩/] w s cap term
      = ((q.matchPartial w s cap (matchPartial_concat_terminates.mp term).1).pmap
          (fun ent rt ↦ r.matchPartial w ent.1 ent.2 rt)
            (matchPartial_concat_terminates.mp term).2).flatten := by
  rw [matchPartial]
  simp only [initMatchPartial]
  step
  run_top!
  rw! [← matchPartial_eq_run]
  rw [Action.concatWait_run (matchPartial_concat_terminates.mp term).2]
...
theorem matchPartial_or (q r : Regex α) (s : Pos w) (cap : Captures w)
    (term : [/⟨q⟩ | ⟨r⟩/].Terminates w s cap)
    : matchPartial [/⟨q⟩ | ⟨r⟩/] w s cap term
      = q.matchPartial w s cap (matchPartial_or_terminates.mp term).1 ++
        r.matchPartial w s cap (matchPartial_or_terminates.mp term).2 := by
  rw [matchPartial]
  simp only [initMatchPartial]
  step
  run_top!
  rw [Action.orWait_run (matchPartial_or_terminates.mp term).2]
  rfl
...
theorem matchPartial_filterEmpty (emp : Bool) (r : Regex α) (s : Pos w) (cap : Captures w)
    ...
    : matchPartial [/⟨r⟩ ‹emp›ε/] w s cap term =
      (r.matchPartial w s cap (matchPartial_filterEmpty_terminates.mp term)).filter
...
theorem matchPartial_star_terminates {t : StarType} {r : Regex α} {s : Pos w}
    {cap : Captures w}
  ...
  | greedy =>
    simp only
    rw [← matchPartial_terminates_iff, matchPartial_or_terminates,
      matchPartial_or_terminates] at ⊢
    rw! [matchPartial_filterEmpty_terminates, matchPartial_concat_terminates,
      matchPartial_filterEmpty_terminates]
    simp [dand_iff_and_forall, matchPartial_empty_terminates, matchPartial_filterEmpty]
  | lazy =>
    simp only
    rw [← matchPartial_terminates_iff, matchPartial_or_terminates,
      matchPartial_or_terminates] at ⊢
...
theorem matchPartial_capture (n : ℕ) (r : Regex α) (s : Pos w) (cap : Captures w)
    ...
    : matchPartial [/(‹n› ⟨r⟩)/] w s cap term =
      (r.matchPartial w s cap (matchPartial_capture_terminates.mp term)).map
