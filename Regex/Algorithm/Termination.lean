...
def stepN (n : ℕ) (st : MatchStack w) : StepResult w (MatchStack w)
  := match n with
  | 0 => Except.ok st
  | n + 1 => st.step >>= stepN n
...
def Terminates' (st : StepResult w (MatchStack w)) :=
...
def step (st : MatchStack.T w) : StepResult w (MatchStack.T w)
...
--def Reachable (st : MatchStack w) (l : Action α → PartialMatches → Prop) :=
