...
omit deq in
@[simp] theorem List.length_extract_icc' (w : List α) (s : Pos w) (t : IccFrom s)
    : (w.extract s.val t.val).length = t.val - s.val :=
  Nat.eq_sub_of_add_eq (List.length_extract_icc w s t)

omit deq in
@[simp] theorem List.length_extract_nat' {w : List α} {s t : ℕ}
    (hs : s ≤ t) (ht : t ≤ w.length)
    : (w.extract s t).length = t - s :=
  Nat.eq_sub_of_add_eq (List.length_extract_nat hs ht)
